	...
	export default {
		...
		methods: {
			init() {
				...
				let  pickPosition = {x: -100000, y: -100000};
				const self = this;
				...
				function init() {
					...
					const geometries = [
						...
						new THREE.SphereGeometry( 0.5*100, 12*10, 8 ),
						new THREE.DodecahedronGeometry( 100 ),
						new THREE.CylinderGeometry( 50, 50, 1, 12 )
					];

					function handleTransform(camera, transform) {
					...
					charts.forEach((el, index) => {
						...
						handleTransform(camera, {k: 0.2, x: 0, y: 0});
